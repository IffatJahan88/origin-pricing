...
export class PricingPage extends BasePage {
    ...
    private readonly plansTableHeader: Locator;
    private readonly comparePlansButton: Locator;
    ...
    constructor(page: Page) {
        ...
        this.plansTableHeader = this.plansTable.getByTestId('table-head');
        this.comparePlansButton = page.getByRole('button', {name: 'Compare plans'})
    ...
    async searchForAddress(address: string) {
        ...
        const addressSuggested = (await this.addressOption.innerText()).replace(/,/g, "").trim().toLowerCase();
        expect(addressSuggested).toBe(address.replace(/,/g, "").trim().toLowerCase());
        await this.addressOption.click();
    }

    async verifyPlansList() {
        await expect(this.comparePlansButton).toBeVisible();
    ...
    async clickPlanLinkInColumn(planName: string, columnName: string): Promise<{ newTab: Page; pdfUrl: string }> {
        await expect(this.plansTable).toBeVisible();
    ...
    async verifyPlanDetailsOpenedInNewTab(newTab: Page, planName: string) {
        ...
        // ✅ Optional: lightweight plan-name check (only if it’s reliably present in URL)
        // Many of your links include the plan name in the URL path? If not, skip this.
        const slug = planName
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, "-")
            .replace(/^-+|-+$/g, "");

        // Only assert this if you’ve confirmed it’s stable for your site
        // await expect(newTab).toHaveURL(new RegExp(slug, "i"));
    }
